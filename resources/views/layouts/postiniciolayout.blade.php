            ...
            <div
                ...
                <nav class="space-y-4 w-full md:w-auto flex md:flex-col justify-around">
                    ...
                    <a href="{{ route('Home') }}" class="flex items-center space-x-3 hover:bg-zinc-700 p-2 rounded">
                        ...
                        <span class="block md:inline">Página Principal</span>
                    </a>
                    <a href="#" id="openModalBusquedaButton"
                        class="flex items-center space-x-3 hover:bg-zinc-700 p-2 rounded">
                        <ion-icon name="search-outline" class="text-2xl"></ion-icon>
                        <span class="block md:inline">Búsqueda</span>
                    </a>
                    <a href="{{ route('Noticias') }}" class="flex items-center space-x-3 hover:bg-zinc-700 p-2 rounded">
                        ...
                        <span class="block md:inline">Noticias</span>
                    </a>
                    <a href="{{ route('Conferencias') }}"
                        class="flex items-center space-x-3 hover:bg-zinc-700 p-2 rounded">
                        <ion-icon name="laptop-outline" class="text-2xl"></ion-icon>
                        <span class="block md:inline">Conferencias</span>
                    </a>
                    <a href="{{ url('/chats') }}" class="flex items-center space-x-3 hover:bg-zinc-700 p-2 rounded">
                        ...
                        <span class="block md:inline">Mensajes</span>
                    </a>
                    <a href="#" id="openModalCrearButton"
                        class="flex items-center space-x-3 hover:bg-zinc-700 p-2 rounded">
                        <ion-icon name="create-outline" class="text-2xl"></ion-icon>
                        <span class="block md:inline">Crear</span>
                    </a>
                    <div class="hidden md:block border-t border-white mt-1 pt-1 w-full"></div>
                    ...
                    @if(auth()->user()->hasRole('Admin'))
                        <a href="{{ route('Administracion') }}"
                            ...
                            <span class="block md:inline">Administrar</span>
                        </a>
                    @endif

                    @if(auth()->user()->hasRole('Usuario'))
                        <a href="#" id="openModal" class="flex items-center space-x-3 hover:bg-zinc-700 p-2 rounded">
...
>>>>>>> 360299770e71f1bc6e9294ba595fb859c66b90ea
                                    ...
                                    <p id="fileName"
                                        class="text-sm text-gray-500 mt-2 text-center truncate max-w-full overflow-hidden text-ellipsis">
            ...
            <div class="flex items-center justify-center min-h-screen">
                <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                    ...
                    <form id="problemForm" method="POST" enctype="multipart/form-data"
                        action="{{ route('informes.store') }}">
                        @csrf
                        <div class="mb-4">
                            <label for="tipo" class="block text-sm font-medium text-gray-700">Tipo de problema que
                                presenta</label>
                            <select type="text" name="tipo" id="tipo" class="w-full border rounded-md p-2" required>
                                ...
                                <option value="Problemas de Compatibilidad entre Navegadores">Problemas de
                                    compatibilidad entre navegadores</option>
                        ...
                        <div class="mb-4">
                            <label for="imagen" class="block text-sm font-medium text-gray-700">Subir Imagen
                                (opcional)</label>
                        ...
                        <div class="flex justify-end">
                            <button type="button" id="cancelProblem"
        ...
        <script>
            function handleResponsiveChanges() {
                // Actualizar visibilidad del div
                const headerDiv = document.querySelector('.flex.items-center.justify-center.h-16.border-b.border-white.w-full');
                if (window.innerWidth < 768) {
                    headerDiv.style.display = 'none'; // Ocultar el div
                } else {
                    headerDiv.style.display = 'flex'; // Mostrar el div
                }

                // Actualizar clases del div responsivo
                const responsiveDiv = document.getElementById('responsiveDiv');
                if (responsiveDiv) {
                    if (window.innerWidth < 768) {
                        responsiveDiv.className = '';
                    } else {
                        responsiveDiv.className = 'w-64';
                    }
                }

                // Actualizar visibilidad de los textos
                const elements = document.querySelectorAll('span');
                elements.forEach(element => {
                    if (window.innerWidth <= 768) {
                        if (element.classList.contains('block') && element.classList.contains('md:inline')) {
                            element.classList.remove('block');
                            element.classList.add('hidden');
                        }
                    } else {
                        if (element.classList.contains('hidden') && element.classList.contains('md:inline')) {
            ...
            handleResponsiveChanges(); // Llamada inicial para establecer el estado
            window.addEventListener('resize', handleResponsiveChanges); // Evento al cambiar el tamaño de la ventana
        </script>
        <div class="flex-1 bg-white p-4 overflow-y-auto h-screen">
            @yield('content')
        </div>
    </div>
